...
<script>
    // ---------- Puentes PHP ----------
    const dataFromPHP = {
        ID_NOTA: <?= json_encode($id_nota ?? null) ?>,
        tienda: <?= json_encode($tienda_nota->tienda ?? '') ?>,
        marca: <?= json_encode($tienda_nota->marca ?? '') ?>,
        pais: <?= json_encode($tienda_nota->pais ?? '') ?>,
        ciudad: <?= json_encode($tienda_nota->ciudad ?? '') ?>,
        num_factura: <?= json_encode($tienda_nota->num_factura ?? '') ?>,
        idTienda: <?= json_encode($_GET['id'] ?? '') ?> // Extraído aquí para evitar redundancia
    };
        ...
        if (!empty($carritoTemporal2)) {
            foreach ($carritoTemporal2 as $r) {
                ...
                $out[] = [
                    ...
                    'num_caja' => $r->num_caja,
                    'bodega' => $r->bodega,
                    'total' => round((float)$r->cantidad * (float)$r->precio_unitario, 2),
                ];
            }
        }
    ...
    const AUTOSAVE_PROPS = new Set([
        'cantidad', 'etiqueta', 'saldo', 'num_factura', 'prenda',
        'composicion', 'precio_unitario', 'tienda', 'marca', 'pais',
        'num_caja', 'bodega'
    ]);
    ...
    const hot = new Handsontable(container, {
        ...
        colHeaders: [
            'id', 'cod', 'cantid', 'etq', 'saldo', 'num_fact', 'prenda',
            'composicion', 'precio_u', 'tienda', 'marca', 'pais', 'num_caja', 'bodega', 'total', 'Acciones'
        ],
        columns: [
            { data: 'id', readOnly: true },
            {
                ...
                renderer: (inst, td, row, col, prop, val) => {
                    td.textContent = val ?? dataFromPHP.ID_NOTA;
                }
            },
            ...
            },
            {
                ...
                renderer: (inst, td, row) => {
                    const r = inst.getSourceDataAtRow(row);
                    td.textContent = r.num_factura || dataFromPHP.num_factura || '';
                }
            },
            { data: 'prenda' }, { data: 'composicion' },
            {
                data: 'precio_unitario', type: 'numeric', numericFormat: { pattern: '0.[00]' }
            },
            { data: 'tienda', renderer: (inst, td, row) => td.textContent = getValue(row, 'tienda', dataFromPHP.tienda) },
            { data: 'marca', renderer: (inst, td, row) => td.textContent = getValue(row, 'marca', dataFromPHP.marca) },
            { data: 'pais', renderer: (inst, td, row) => td.textContent = getValue(row, 'pais', dataFromPHP.pais) },
            ...
            },
            {
                ...
                renderer(inst, td, row) {
                    ...
                    td.innerHTML = `<button class="btn btn-outline-danger btn-sm btn-del" data-row="${row}"><i class="bi bi-trash me-1"></i>Eliminar</button>`;
                }
            }
        ],
        rowHeaders: true,
