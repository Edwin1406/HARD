...
const hot = new Handsontable(container, {
    ...
    columns: [
        ...
        { 
            ...
            readOnly: true,
            renderer: (inst, td, row, col, prop, val) => td.textContent = val ?? (ID_NOTA ?? ''),
        },
        ...
        { 
            data: 'tienda',
            renderer: (inst, td, row) => td.textContent = row.tienda || tienda || '',
        },
        { 
            data: 'marca',
            renderer: (inst, td, row) => td.textContent = row.marca || marca || '',
        },
        { 
            data: 'pais',
            renderer: (inst, td, row) => td.textContent = row.pais || pais || '',
        },
    ...
    afterChange(changes, source) {
        ...
        rowsToUpdate.size && rowsToUpdate.forEach(r => recalcRow(r));
        rowsToUpdate.size && maybeAutosave([...rowsToUpdate]);
    },
    afterPaste() {
...
function recalcRow(rowIndex) {
    ...
    r.etiqueta = r.etiqueta?.trim() || '';
    r.saldo = r.saldo?.trim() || '';
    r.num_factura = Number(r.num_factura) || 0;
    r.prenda = r.prenda?.trim() || '';
    r.composicion = r.composicion?.trim() || '';
    r.precio_unitario = Number(r.precio_unitario) || 0;
    r.tienda = r.tienda?.trim() || '';
    r.marca = r.marca?.trim() || '';
    r.pais = r.pais?.trim() || '';
    r.num_caja = Number(r.num_caja) || 0;
    r.bodega = r.bodega?.trim() || '';
